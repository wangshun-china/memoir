...
use super::service::{
    create_session, finish_session, get_session, list_messages, post_message,
    CreateInterviewRequest, InterviewMessage, InterviewSession, PostMessageRequest,
    PostMessageResponse,
};
...
async fn create_interview(
    ...
    let (session, opening) = create_session(&state.pool, user.user_id, memoir_id, body).await?;
